    ...
    def validate_user_response(self):
        while self.user_response.lower() != 'y' and self.user_response.lower() != 'n':
            self.user_response = input(
                "Unrecognized input. Continue translating? (y/n): ")

    def run(self):
        while True:
            self.user_text = input("Enter text to convert to E language:\n\n")
            print()
            self.translated_text = self.language_translator.translate(
                self.user_text)
            ...
            print()
            self.user_response = input(
                "Translated text has been copied to clipboard. Continue translating? (y/n): ")
